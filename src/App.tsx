...
import { useMemo, useState } from "react";
import { type LineItem, calcTotals, fmtEUR } from "./lib/invoice";
import { savePdf } from "./lib/savePdf.ts";
import { buildInvoicePDF } from "./lib/pdf.ts";
...
// helpers IBAN locales (si ya los tienes en lib/iban, puedes importarlos)
...
export default function App() {
  ...
  // Tabs móvil
  const [activeTab, setActiveTab] = useState<"factura" | "datos" | "conceptos" | "impuestos">("factura");
  const tabs = [
  ...
  const addItem = () => setItems((prev) => [...prev, { descripcion: "", cantidad: 1, precio: 0 }]);
...
function Row({ label, value, red = false, big = false }: { label: string; value: string; red?: boolean; big?: boolean }) {
  return (
    <div className={`flex items-center justify-between ${big ? "text-lg" : ""}`}>
      <span>{label}</span>
      <strong className={red ? "text-rose-600" : ""}>{value}</strong>
