...
export function buildInvoicePDF(opts: {
  ...
  irpfPct: number;
  iban?: string;
  ajustes?: { concepto: string; importe: number }[];
  totalConAjustes?: number;
}) {
  const { numero, fecha, emisor, cliente, items, ivaPct, irpfPct, iban, ajustes = [],
  totalConAjustes, } = opts;
  const totals = calcTotals(items, ivaPct, irpfPct);
  const ajusteNet = ajustes.reduce((s, a) => s + (Number(a?.importe) || 0), 0);
  ...
  // ── Ajustes opcionales (si existen)
if (ajustes.length > 0) {
  // pequeño espacio visual
  y += 6;

  // filas por cada ajuste
  for (const a of ajustes) {
    const label = (a?.concepto?.trim()?.length ? a.concepto : "Ajuste");
    const importe = Number(a?.importe) || 0;
    drawAmountLine(label, fmtEUR(importe), importe < 0 ? { color: [220, 38, 38] } : {});
  }

  // subtotal de ajustes
  drawAmountLine("Subtotal ajustes", fmtEUR(ajusteNet));
}

// ── Total final (override si viene de UI)
const finalTotal = (typeof totalConAjustes === "number")
  ? totalConAjustes
  : totals.total;

  doc.setFont("helvetica", "bold");
  drawAmountLine("TOTAL", fmtEUR(finalTotal), { bold: true });
  doc.setFont("helvetica", "normal");
